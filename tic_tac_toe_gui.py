...
class TicTacToeGUI:
    ...
    def setup_gui(self):
        ...
        # Difficulty selection with dark theme
        difficulty_frame = tk.Frame(self.root, bg="#0a0a0a")
    ...
    def ai_move(self):
        ...
        if difficulty == "Easy":
            ...
            # 70% optimal moves, 30% random for medium difficulty
            import random
    ...
    def process_ai_move(self, row: int, col: int):
        ...
        if is_over:
            ...
            self.status_label.config(text="Your turn! Click any cell to make a move.")
            self.enable_board()
        ...
        for i in range(3):
            for j in range(3):
                if self.game.board[i][j] == ' ':
                    ...
                    # Add hover effect
                    button.bind("<Enter>", lambda e, btn=button: btn.config(bg='#2a2a4e'))
                    button.bind("<Leave>", lambda e, btn=button: btn.config(bg='#1a1a2e'))
    ...
    def handle_game_end(self, result):
        ...
        # Show message box with enhanced styling
        self.root.after(1000, lambda: messagebox.showinfo(
            "Game Over", 
            message + "\n\nClick 'NEW GAME' to play again!"
        ))
    
    def flash_winner_cells(self):
        """Add a flashing effect to winning cells."""
        # This could be enhanced to highlight the winning line
        for i in range(3):
            for j in range(3):
                if self.game.board[i][j] == self.game.human_player:
                    ...
                    # Flash effect
                    button.config(bg='#00ff88')
                    self.root.after(200, lambda b=button, orig=original_bg: b.config(bg=orig))
    ...
    def new_game(self):
        """Start a new game with neon-styled reset."""
        self.game = TicTacToeGame()
        self.ai = MinimaxAI(self.game.ai_player, self.game.human_player)
